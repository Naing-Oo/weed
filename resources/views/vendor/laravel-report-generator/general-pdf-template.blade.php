				...
				<div class="head-content">
					<table cellpadding="0" cellspacing="0" width="100%" border="0">
						...
						@foreach($headers['meta'] as $name => $value)
							...
							<tr>
							@endif
								<td class="middle"><span style="color:#808080;">{{ $name }}</span>: {{ ucwords($value) }}</td>
							@if ($metaCtr % 2 == 1)
							</tr>
							@endif
							<?php $metaCtr++; ?>
                    ...
                    @foreach($query->when($limit, function($qry) use($limit) { $qry->take($limit); })->cursor() as $result)
						<?php
							...
							if ($groupByArr) {
								...
								foreach ($groupByArr as $groupBy) {
									if (is_object($columns[$groupBy]) && $columns[$groupBy] instanceof Closure) {
										...
										if ($columns[$groupBy] == 'agent_name'){
											...
											// echo $currentGroupByData[$groupBy];
								...
								if ($isOnSameGroup === false || $isOnSameAgent === false) {
